...
def run_search():
    ...
    with open(output_file, 'r') as file:
        extracted_domains = file.read().splitlines()

    # Call evaluator.py to evaluate the domains
    subprocess.run(['python', 'evaluator.py'])

    # Read evaluated domains from the file
    evaluated_domains = {}
    with open('evaluated-domains.txt', 'r') as file:
        for line in file:
...
def extract_domains_from_query(query):
    extracted_domains = ['example.com', 'trusted-site.com', 'random-site.net']
    return extracted_domains

# Example query or input data
example_query = "Some text containing domains like example.com and trusted-site.com"

# Extract domains from the query
extracted_domains = extract_domains_from_query(example_query)

# Create a dictionary to store ratings for each domain
domain_ratings = {}
